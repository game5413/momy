...
function createDefs (collections, dbName, opts) {
  ...
  return Object.keys(collections).map(name => {
    ...
    const tableAlias = /(\s)+as+(\s)/g
    if (tableAlias.test(tableName)) {
      tableName = tableName.split(tableAlias)
      collectionName = tableName[0]
      tableName = tableName[tableName.length - 1]
    }
    return {
      name: collectionName,
      ns: `${dbName}.${collectionName}`,
      distName: opts.prefix + tableName,
      idName,
...
function convertCase (str, fieldCase) {
  return fieldCase === 'camel' ? textCase.camelCase(str)
    : fieldCase === 'snake' ? textCase.snakeCase(str)
    : str
}
