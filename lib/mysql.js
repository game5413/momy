...
class MySQL {
  ...
  constructor (url) {
    ...
    this.con = null
    const ddl = 'CREATE TABLE IF NOT EXISTS `mongo_to_mysql` (service varchar(20), timestamp BIGINT);'
  ...
  createTable(name, fields, callback) {
    ...
    const ddl = `CREATE TABLE IF NOT EXISTS \`${name}\` (${columns.join(', ')});`
  ...
  bulkCreateTable(defs) {
    const list = cloneDeep(defs)
    if (!list.length) return
    const def = cloneDeep(list[0])
    list.shift()
